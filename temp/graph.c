...
typedef void abstract_reachable_list;
...
{
	// TO BE CHANGED!!!
	abstract_reachable_list *reachable_nodes;
	void *key;
	void *data;
};
...
// NEW LIST
struct oak_graph_reachable_list_node_s
{
	oak_graph_node_t element;
	struct oak_graph_reachable_list_node_s *next;
};


typedef struct oak_graph_reachable_list_node_s* oak_graph_reachable_list_node_t;
...
oak_graph_node_t oak_graph_create_node(void* get_key_p())
{
	...
	void* glist = new_node->reachable_nodes;
	(oak_graph_node_list_s *)glist->size = 0;
